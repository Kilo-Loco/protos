...
struct ContributionsGraph: View {
    
    static let boxSize: CGFloat = 15
    static let spacing: CGFloat = 4
    let rows = Array(repeating: GridItem(.fixed(boxSize), spacing: spacing), count: 7)
    
    let days: [Contribution]
    let selectedDay: (Contribution) -> Void
